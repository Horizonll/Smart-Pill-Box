...
class OCRProcessor:
    def __init__(self):
        self.ocr = PaddleOCR(use_angle_cls=True, lang="ch", show_log=False, enable_mkldnn=True)
...
class TTSProcessor:
    def __init__(self):
        os.chdir("/home/pill/smart-pill-box")
        self.chat = Chat()
        self.chat.load(source="custom", custom_path="tts", compile=False)
        torch.manual_seed(104302)
...
class ImageProcessingThread(threading.Thread):
    ...
    def generate_audio(self, text):
        ...
        with condition:
            num_files = len(parts)
            condition.notify_all()
        params_infer_code = Chat.InferCodeParams(prompt="[speed_0]", spk_emb=self.tts_processor.spk)
        for i, part in enumerate(parts):
            if self._stop_event.is_set():
                return
            wavs = self.tts_processor.chat.infer(part, params_infer_code=params_infer_code)
    ...
    def split_text(self, text, max_length=100):
        ...
        num_to_chinese = str.maketrans("0123456789", "零一二三四五六七八九")
        text = text.translate(num_to_chinese)
...
def _init():
    global tts_processor, ocr_processor, num_files, condition, sound_list
...
def read_news():
    global num_files, condition, sound_list, ocr_processor, tts_processor, image_processing_thread, audio_playback_thread
...
def add_medicine():
    ...
    time2 = time.time()
    print(f"OCR耗时: {time2 - time1:.2f}s")
    print("提取到的文本:")
...
def get_audio_files():
    ...
    if not usb:
        return audio_files
    directory = f"/medipill/pill/{usb[1]}"
...
def copy_audio_file(files):
    ...
    usb = get_usb()
    for file in files:
        shutil.copy(f"/medipill/pill/{usb[1]}/{file}", f"audio/{file}")
